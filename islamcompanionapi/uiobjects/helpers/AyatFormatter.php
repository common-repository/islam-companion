...
{    
    ...
    {      
        ...
        for ($count = 0; $count < count($ayat_list); $count++) {           
            ...
            $meta_data['sura']                            = $ayat_list[$count]['sura_data']['tname'];
             /** The sura id is set */
            $meta_data['sura_id']                         = $ayat_list[$count]['sura_data']['sindex'];
            /** The sura ayat id is set */
            $meta_data['sura_ayat_id']                    = $ayat_list[$count]['sura_ayat_id'];
            /** The meta data for the ayat text */
            $meta_data['meta_data']                       = " (" . $meta_data['sura'] . " " . $meta_data['sura_id'] . ":" .$meta_data['sura_ayat_id']. ")";
        ...
        /** The template parameters are set */
        $template_parameters                              = array("content" => $formatted_text_html, "list_class" => $parameters['css_class'] . " " . $parameters['css_attributes']);
        /** The verse text */
    ...
    private function FormatAyat($text, $options, $parameters) 
    {
        ...
        if (is_array($options) && in_array('copy', $options['tools_list'])) {
            ...
            $copy_text                                            = $text . " (" . $parameters['meta_data']['sura_data']['tname'] . " " . $parameters['meta_data']['sura'] . "-" . $parameters['meta_data']['sura_ayat_id'];
    ...
    private function FormatAsPlainText($ayat_list, $options, $parameters) 
    {
        ...
        $ayat_text                    = $this->GetComponent("template")->Render("div", array("content" => $ayat_text, "id" => "holy-quran-text", "css_class" => $parameters['css_class'] . " " . $parameters['css_attributes']));
