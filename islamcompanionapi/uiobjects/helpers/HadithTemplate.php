...
trait HadithTemplate
{
    ...
    public function PopulateEmailSubscriberTemplate($hadith_text_html, $meta_data)
    {
        ...
        $url_query                         = http_build_query(array("parameters" => array("email_addresses" => $meta_data['email_address'], "type" => "Hadith", "source" => "email_link")));
        /** The unsubscribe link. It takes the user to the Hadith navigator page */
        $unsubscribe_link                  = $this->GetConfig("general", "site_url") . "index.php?module=IslamCompanion&option=remove_subscribers&" . $url_query;
        /** The template parameters are set */
