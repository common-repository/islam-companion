...
trait AyatTemplate
{
    ...
    public function PopulateEmailSubscriberTemplate($verse_text_html, $meta_data)
    {
        ...
        /** The url query string is built */
        $url_query                         = http_build_query(array("parameters" => array("email_addresses" => $meta_data['email_address'], "type" => "Holy Quran", "source" => "email_link")));
        /** The language code */
        $language_code                     = $language_data['language_code'];
        /** The text direction */
        $text_direction                    = ($language_data['rtl']) ? 'rtl' : 'ltr';
        /** The unsubscribe link. It takes the user to the Hadith navigator page */
        $unsubscribe_link                  = $this->GetConfig("general", "site_url") . "index.php?module=PakJiddat&option=remove_subscribers&" . $url_query;
        /** The template parameters are set */
        $template_parameters               = array("body" => $verse_text_html, "title" => "Islam Companion", "css_tags" => $css_tags, "javascript_tags" => $javascript_tags, "font_tags" => $font_tags, "language" => $meta_data['language'], "narrator" => $meta_data['narrator'], "sura" => $meta_data['sura'], "start_ayat" => $meta_data['start_ayat'], "end_ayat" => $meta_data['end_ayat'], "ayat_keywords" => $meta_data['keywords'], "ayat_times" => $meta_data['times'], "ayat_count" => $meta_data['number_of_results'], "email_addresses" => $meta_data['email_address'], "ayat_order" => $meta_data['order'], "unsubscribe_link" => $unsubscribe_link, "language_code" => $language_code, "text_direction" => $text_direction);
