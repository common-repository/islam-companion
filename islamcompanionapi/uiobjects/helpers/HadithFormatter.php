...
{    
    ...
    private function FormatAsParagraph($hadith_list, $options, $parameters) 
    {
        ...
        for ($count = 0; $count < count($this->data['hadith_list']); $count++) 
        {
            ...
            /** The meta data is added if required */
            if (in_array("meta data", $options['tools_list'])) {
                /** The parameters used to format the hadith */
                $formatting_parameters  = array("meta_data" => $this->data['hadith_list'][$count]['source'] . " - " . $this->data['hadith_list'][$count]['book'] . " - " . $this->data['hadith_list'][$count]['hadith_number'] . ":" . $this->data['hadith_list'][$count]['id'] . " (" . $this->data['hadith_list'][$count]['title'] . ")");
                /** The hadith text. It is formatted by adding paragraph tags around every 3 sentences */
                $hadith_text            = $this->FormatText($hadith_text, array("add paragraphs", "meta data"), $formatting_parameters);
            }
            else {
                /** The hadith text. It is formatted by adding paragraph tags around every 3 sentences */
                $hadith_text            = $this->FormatText($hadith_text, array("add paragraphs"), array());
            }
            /** The html id is added to the hadith number */
