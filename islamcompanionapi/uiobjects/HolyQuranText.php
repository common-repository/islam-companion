...
class HolyQuranText extends \Framework\Object\UiObject
{
     ...
     *    user_interface => string [email~plain text~list~navigator~search results] the user interface where the verse text will be shown
     */
    ...
    public function Display()
    {
        ...
        if ($this->data['user_interface'] == 'email') {
            ...
            $total_results        = $this->data['total_results'];
            /** The result count */
            $result_count         = count($this->data['ayat_list']);
            ...
            $ayat_text_html       = $this->FormatAyas($this->data['ayat_list'], "list", $this->data['parameters'], $options);
            /** The next ayat id */
            $next_ayat_id         = ($this->data['ayat_list'][$result_count -1 ]['id'] + 1);
            /** The first ayat */
            $first_ayat           = $this->data['ayat_list'][0]['sura_ayat_id'];
            /** The last ayat */
            $last_ayat            = $this->data['ayat_list'][$result_count-1]['sura_ayat_id'];
            /** The meta data for the ayat text html */
            $meta_data            = array("keywords" => $this->data['parameters']['search_text'], "number_of_results" => $this->data['parameters']['number_of_results'] . " " . $this->data['parameters']['result_type'], "narrator" => $this->data['narrator'], "language" => $this->data['language'], "sura" => $this->data['sura_data']['tname'] . " (" . $this->data['sura_data']['ename'] . ")", "start_ayat" => $first_ayat, "end_ayat" => $last_ayat, "order" => $this->data['parameters']['order'], "times" => $this->data['parameters']['times'], "email_address" => $this->data['parameters']['email_address']);
